...
- trending：过去 24–72 小时 HN/Reddit/GitHub/X 四个热榜上与 AI/GPU/推理相关的条目，哪些热度数字之外还有可核验的一手增量事实，哪些只是传播噪音？
- chips：本周哪份一手文件（产能、订单、良率、互连、镜像）实际改变了一款加速器的可交付量或每瓦推理成本？
- models-agents：哪个模型或 agent 产品变化把"单位任务成本"或"权限/恢复边界"变成了可测量的差异？
- inference-systems：本周哪里出现"官方推荐配置在新硬件、新并发或新精度下反优化"的可复现证据？
- papers-oss：哪项新工作把负结果或失败条件写成了后续工作可以直接引用的锚点？
- china-industry：哪家中文公司的一手材料本周把国产部署的真实规模、订单或兼容性边界变成可核验数字？
- community：哪个被广泛转发的性能/成本主张本周被具名从业者用对照实验压上了反例数据？
- deepseek-radar：DeepSeek 生态本周是否出现新的权重、kernel 代际或下游集成默认值变化，而不是 RC 逐日 bump？
